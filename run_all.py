...
if batch:
	...
	masses = [200, 250, 300, 350, 400, 500]  # 600
	nToys = 200
		...
		for nsigfac in nsigfacs:
			for mass in masses:
				for jobId in range(nJobs):
					...
					cmd = 'qsub  -q cms -V -o %s -e %s  -j oe  -l walltime=48:00:00,cput=48:00:00  -N %s  -F "%s %s %d %d %d %d"  %s' \
								% ("logs", "logs", cname, ch, f, mass, nToys, nsigfac, jobId, script)
					print(cmd)
